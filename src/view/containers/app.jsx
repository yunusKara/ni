...
export default React.createClass({
  ...
  handlePresence: function() {
    //Runtime.updateEntityKnowledge(0, 'sensors', {presence: event.target.checked });
    devices.setPresence( event.target.checked );
    //console.log("presence state = " + event.target.checked);
  },
  ...
  getPlanning: function() {
    return Runtime.getEntityKnowledge(0)
    .then((kb)=>{
      return kb.knowledge.planning;
    })
    .catch(()=>{
  ...
  start:function() {
    ...
    Runtime.createSimulation( user, project, branch, appID, appSecret )
    .then(() => {
      this.setState( {simID: Runtime.simID} );
      Runtime.doWS();
    ...
    .then(() => {
      ...
      var planWE = [{time:6*60,value:20},
                  ...
                  {time:20*60,value:20},
                  {time:23*60,value:18}]
      return Runtime.createEntity( 'agent/Robot.bt', {time:MapOfTime(ActionsStore.getTime()),planning:{workday:planWD,weekend:planWE}} );
    })
    .then(()=> {
      this.setState( {ready: true} );
      this.handleTimeInit(2);
      Runtime.doUpdate();
      console.log(Runtime.simID);
